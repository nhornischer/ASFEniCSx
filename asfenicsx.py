...
class ASFEniCSx:
    ...
    def evaluate_gradients(self, info = False, **kwargs):
    ...
    def covariance(self, gradients : np.ndarray, info = False):
        ...
        if info:
            import tqdm.autonotebook
            progress = tqdm.autonotebook.tqdm(desc="Calculating Covariance", total=self.samples.M)
        for i in range(self.samples.M):
    ...
    def random_sampling_algorithm(self, info = False):
        ...
        if not hasattr(self, 'gradients'):
            self.evaluate_gradients(info)
        
        # Construct the covariance matrix
        convariance_matrix = self.covariance(self.gradients, info)
    ...
    def bootstrap(self, M_boot : int, info = False):
        ...
        if not hasattr(self, 'eigenvalues'):
            self.random_sampling_algorithm(info)
        ...
        for i in range(M_boot):
            ...
            S, U = self.calculate_eigenpairs(self.covariance(bootstrap_replicate, info))
